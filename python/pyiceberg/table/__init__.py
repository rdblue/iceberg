...
def get_manifest(manifest: ManifestFile, io: FileIO, partition_filter: Callable[[DataFile], bool]) -> List[DataFile]:
...
class DataScan(TableScan["DataScan"]):
    ...
    def plan_files(self) -> Iterator[FileScanTask]:
        ...
        with ThreadPool() as pool:
            return chain(
                *pool.starmap(
                    get_manifest, [(manifest, io, partition_evaluators[manifest.partition_spec_id]) for manifest in manifests]
                )
            )
