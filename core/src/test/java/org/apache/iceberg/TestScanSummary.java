...
public class TestScanSummary extends TableTestBase {
  ...
  private long waitUntilAfter(long timeMillis) {
    long time = System.currentTimeMillis();
    while (time <= timeMillis) {
  ...
  @Test
  public void testScanSummaryWithFirstSnapshot() {
    long t0 = System.currentTimeMillis();
    waitUntilAfter(t0);
    ...
    Assert.assertEquals(1, partitions.get("data_bucket=0").recordCount());
    Assert.assertEquals(10, partitions.get("data_bucket=0").totalSize());
  ...
  public void testMetadataOnlyScanSummary() {
    ...
    Sets.newHashSet(concat(transform(table.snapshots(), snap -> snap.allManifests())))
