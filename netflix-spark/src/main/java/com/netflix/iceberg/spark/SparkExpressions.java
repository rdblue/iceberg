...
public class SparkExpressions {
  ...
  public static org.apache.iceberg.expressions.Expression convert(Expression expr) {
    ...
    if (op != null) {
      switch (op) {
        ...
        case NOT:
          org.apache.iceberg.expressions.Expression child = convert(((Not) expr).child());
          if (child != null) {
            return not(child);
          }
          return null;
        case AND:
          ...
          org.apache.iceberg.expressions.Expression andLeft = convert(andExpr.left());
          org.apache.iceberg.expressions.Expression andRight = convert(andExpr.right());
        ...
        case OR:
          Or orExpr = (Or) expr;
          org.apache.iceberg.expressions.Expression orLeft = convert(orExpr.left());
          org.apache.iceberg.expressions.Expression orRight = convert(orExpr.right());
  ...
  private static org.apache.iceberg.expressions.Expression convertIn(Expression expr,
                                                                     Collection<Object> values) {
    if (expr instanceof Attribute) {
      Attribute attr = (Attribute) expr;
      org.apache.iceberg.expressions.Expression converted = alwaysFalse();
      for (Object item : values) {
        converted = or(converted, equal(attr.name(), valueFromSpark(item)));
      }
      return converted;
    }
  ...
  private static org.apache.iceberg.expressions.Expression convert(Operation op,
                                                                   Expression left,
                                                                   Expression right) {
    ...
    if (attrPair != null && attrPair.first() != null && attrPair.second() != null) {
  ...
  private static Object valueFromSpark(Object value) {
  ...
  private static Object valueFromSpark(Literal lit) {
    return valueFromSpark(lit.value());
  }
  ...
  private static org.apache.iceberg.expressions.Literal<Long> tsLiteral(long timestampMicros) {
    return org.apache.iceberg.expressions.Literal
        .of(timestampMicros)
        .to(TimestampType.withoutZone());
  }

  private static org.apache.iceberg.expressions.Expression filter(
  ...
  public static Expression convert(
      ...
      Schema schema, boolean caseSensitive) {
    return visit(
        Binder.bind(schema.asStruct(), filter, caseSensitive),
  ...
  private static class ExpressionToSpark extends ExpressionVisitors.
    ...
    public <T> Expression lt(BoundReference<T> ref,
                             org.apache.iceberg.expressions.Literal<T> lit) {
