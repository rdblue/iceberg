...
class PartitionKey implements StructLike {
  ...
  private static Accessor<InternalRow> newAccessor(int p, Type type) {
    switch (type.typeId()) {
      case STRING:
        return new StringAccessor(p, SparkSchemaUtil.convert(type));
      case DECIMAL:
        return new DecimalAccessor(p, SparkSchemaUtil.convert(type));
      case BINARY:
        return new BytesAccessor(p, SparkSchemaUtil.convert(type));
      default:
        return new PositionAccessor(p, SparkSchemaUtil.convert(type));
    }
  }
