...
public class TestFilteredScan {
  ...
  public void testUnpartitionedCaseInsensitiveIDFilters() {
    ...
    try {
      ...
      for (int i = 0; i < 10; i += 1) {
        ...
        pushCaseInsensitiveFilters(reader, Expressions.equal("ID", i));
  ...
  private void pushCaseInsensitiveFilters(DataSourceReader reader,
                           org.apache.iceberg.expressions.Expression... filters) {
    pushFilters(reader, filters, false);
  }

  private void pushFilters(DataSourceReader reader,
                           org.apache.iceberg.expressions.Expression... filters) {
    pushFilters(reader, filters, true);
  }
  ...
  private void pushFilters(DataSourceReader reader,
    ...
    for (int i = 0; i < filters.length; i += 1) {
      expressions[i] = SparkExpressions.convert(filters[i], SCHEMA, caseSensitive);
    }
