...
class CommentUpdate implements UpdateComment {
    ...
    public void commit() {
        ...
        ViewDefinition viewDefinition = ViewDefinition.of(baseDefinition.sql(), apply(),
                baseDefinition.sessionCatalog(), baseDefinition.sessionNamespace());
        // When metacat alters a column comment, it goes through this code path. Genie-id is not available
        // in that case. When an engine (e.g. Spark) alters a column comment, it turns into a 'replace' view
        ...
        BaseVersion version = new BaseVersion(base.currentVersionId() + 1, base.currentVersionId(), System.currentTimeMillis(),
                summary, viewDefinition);
        ViewVersionMetadata update = ViewVersionMetadata.newViewVersionMetadata(version, base.location(), viewDefinition,
    ...
    private static Schema applyChanges(Schema schema, Map<Integer, Types.NestedField> updates) {
        Types.StructType struct = TypeUtil
                .visit(schema, new ApplyChanges(updates))
